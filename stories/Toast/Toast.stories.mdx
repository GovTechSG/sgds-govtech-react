...
<Meta
  ...
  argTypes={{
    ...
    status : {
      description: 'Adds CSS styling to `<Toast />` based on the defined status'
    }
  }}
/>
...
<Canvas>
  <Story name="SGDS Toast" args={{isSGDS: true}}>{Template.bind({})}</Story>
  <Story name="Bootstrap Toast" args={{isSGDS: false}}>{Template.bind({})}</Story>
</Canvas>
...
<Canvas>
  <Story name="Success Toast" args={{status: 'success'}}>{Template.bind({})}</Story>
  <Story name="Danger Toast" args={{status: 'danger'}}>{Template.bind({})}</Story>
  <Story name="Warning Toast" args={{status: 'warning'}}>{Template.bind({})}</Story>
</Canvas>
