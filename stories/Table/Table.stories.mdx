...
Sorting on a column is enabled by adding the `sort` property. The sorting algorithm is based on javascript array.sort() method.
...
<Canvas>
  <Story name="removableSort" args={{ sort: true, removableSort: true }}>
    {Basic.bind({})}
  </Story>
</Canvas>
