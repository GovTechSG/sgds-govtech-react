...
describe('<Pagination>', () => {
  ...
  it('renders 3 page numbers as dataLength = 20', () => {
    const { getByText, queryByText } = render(
      <Pagination limit={3} dataLength={20} />
    );
  ...
  it('limit >= pages.length, all page number should show and remain fix throughout navigation', () => {
    const { container, rerender } = render(
      <Pagination limit={4} dataLength={3} itemsPerPage={1} currentPage={1} />
    );
    const textWhenThreePg = 'Previous123Next';
    // limit 4 , 3 pages
    expect(container.textContent).toContain(textWhenThreePg);
    rerender(
      <Pagination limit={4} dataLength={3} itemsPerPage={1} currentPage={2} />
    );
    expect(container.textContent).toContain(textWhenThreePg);
    rerender(
      <Pagination limit={4} dataLength={3} itemsPerPage={1} currentPage={3} />
    );
    expect(container.textContent).toContain(textWhenThreePg);

    const textWhenFivePg = 'Previous12345Next';
    rerender(
      <Pagination limit={5} dataLength={5} itemsPerPage={1} currentPage={1} />
    );
    expect(container.textContent).toContain(textWhenFivePg);
    rerender(
      <Pagination limit={5} dataLength={5} itemsPerPage={1} currentPage={2} />
    );
    expect(container.textContent).toContain(textWhenFivePg);
    rerender(
      <Pagination limit={5} dataLength={5} itemsPerPage={1} currentPage={3} />
    );
    expect(container.textContent).toContain(textWhenFivePg);
    rerender(
      <Pagination limit={5} dataLength={5} itemsPerPage={1} currentPage={4} />
    );
    expect(container.textContent).toContain(textWhenFivePg);
    rerender(
      <Pagination limit={5} dataLength={5} itemsPerPage={1} currentPage={5} />
    );
    expect(container.textContent).toContain(textWhenFivePg);
  });
...
describe('ellipsis behaviour', () => {
  ...
  it('renders first ellipsis when ellipsisOn = true', () => {
    const { queryAllByText, rerender } = render(
      <Pagination dataLength={50} currentPage={1} ellipsisOn={true} />
    );
    // no first ellipsis
    //ToDO : fix
...
describe('showFirstPage and showLastPage behaviour', () => {
  const setCurrentPage = jest.fn();

  it('when set showFirstPage to true, first page will always be available to select', () => {
  ...
  it('when set showFirstPage to false, first page will not be available to select', () => {
  ...
  it('when set showLastPage to true, last page will always be available to select', () => {
    const { getByText, rerender } = render(
      <Pagination
        limit={3}
        dataLength={20}
  ...
  it('when set showLastPage to false, last page will not be available to select', () => {
    const { queryByText, rerender } = render(
      <Pagination
        limit={3}
        dataLength={20}
