...
import { SGDSWrapper } from '../../src/components/ThemeProvider/ThemeProvider';
...
describe('<Button>', () => {
  ...
  it('Should have type=button by default', () => {
    const $button = mount(<Button>Title</Button>).find('button[type="button"]');
    /* .getDOMNode()
      .getAttribute('type')
      .should.equal('button'); */
  ...
  it('Should show the type if passed one', () => {
    const $button = mount(<Button type="submit">Title</Button>).find(
      'button[type="submit"]'
    );
    expect($button).toBeDefined();
  });
  ...
  it('Should not have default type=button when "as" is used', () => {
    const $button = mount(<Button as="div">Title</Button>).find(
      'button[type="button"]'
    );
  ...
  it('Should remove default variant', () => {
    const $button = mount(<Button variant={null}>Title</Button>).find(
      `.btn-primary`
    );
  ...
  it('Should not output null variant', () => {
    const $button = mount(<Button variant="">Title</Button>).find(`.btn-null`);
    expect($button.length).toEqual(0);
  });

  it('Should not output empty variant', () => {
    const $button = mount(<Button variant="">Title</Button>).find(`.btn-`);
    expect($button.length).toEqual(0);
  });
